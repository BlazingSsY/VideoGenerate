...
from app.agent_service import validate_plan
...
class AgentSecurityTests(unittest.TestCase):
    ...
    def test_fallback_plan_splits_long_target_into_legal_segments(self):
        response = self.client.post("/api/agent/plans", json={"surface": "studio", "user_input": "产品宣传片", "target_duration": 30})
        self.assertEqual(response.status_code, 201)
    ...
    def test_fallback_plan_does_not_create_too_short_last_segment(self):
        response = self.client.post(
            "/api/agent/plans",
            json={"surface": "studio", "user_input": "短片", "target_duration": 16},
        )
        self.assertEqual(response.status_code, 201)
        generated = [node for node in response.json()["plan"]["nodes"] if node["type"] == "generate"]
    ...
    def test_accept_with_execute_creates_recoverable_run_and_tasks(self):
        response = self.client.post(
            "/api/agent/plans",
            json={"surface": "studio", "user_input": "产品宣传片", "target_duration": 30},
        )
        self.assertEqual(response.status_code, 201)
        turn_id = response.json()["id"]
        with patch("app.routers.agent.spawn_run") as spawn:
            accepted = self.client.post(
                f"/api/agent/plans/{turn_id}/accept",
                json={"execute": True},
            )
        ...
        with self.Session() as db:
            ...
            self.assertEqual(len(tasks), len(response.json()["plan"]["nodes"]))
